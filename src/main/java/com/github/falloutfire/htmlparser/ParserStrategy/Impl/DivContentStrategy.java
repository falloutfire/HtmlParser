...
public class DivContentStrategy implements ParserStrategy {

    @Override
    public Page parse(Page page) {
        CleaningContent.clean(page);

        Elements header = page.getDocument().select("div[class~=(?i)header(?i)]");
        if (!header.isEmpty()) {
            Element title = header.select("h1").first();
            if (title != null) {
                page.setArticle(new Elements(title, page.getArticle().first()));
                return page;
            }
        } else {
            Element title = page.getDocument().select("h1").first();
            if (title != null) {
                page.setArticle(new Elements(title, page.getArticle().first()));
                return page;
            } else {
                title = page.getDocument().select("h2").first();
                page.setArticle(new Elements(title, page.getArticle().first()));
                return page;
            }
        }
